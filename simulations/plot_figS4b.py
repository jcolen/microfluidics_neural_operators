...
k_elast = 0.02
fname = f"data/twogaussians2_k_{k_elast}.h5"
cmap = plt.cm.Reds
...
ax.invert_yaxis()
p = ax.pcolormesh(xshift - xx, tt[:201]/1000, drho_t[:201,:], shading="gouraud",
...
fig.savefig(f"figs/screened_2gaussians2_k{k_elast}_kymo_c.png", transparent=True, dpi=600)
...
for i, t in enumerate(ks[1:]):
    ax.plot(-xx2 + xshift, uu2[t, :], "-", color="0.85", lw=0.75)
...
fig.savefig(f"figs/screened_2gaussians2_k{k_elast}_c.pdf", transparent=True)
...
ax.set_xlabel(r"$x-ct$")
ax.yaxis.set_ticks([0., 0.1, 0.2, 0.3])
ax.yaxis.set_ticklabels(["0", "", "0.2", ""])
ax.set_ylabel(r"$\delta\rho$", labelpad=0)
...
fig.savefig("figs/cbar_rho_gaussians2.pdf", transparent=True)
...
fig.savefig("figs/cbar_times_gaussians2.pdf", transparent=True)
